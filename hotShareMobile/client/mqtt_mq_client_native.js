...
if(Meteor.isClient && withNativeMQTTLIB){
    ...
    initMQTT = function(clientId){
        ...
        sendMqttMessage=function(topic,message,callback){
            ...
            mqtt.publish({topic:topic, message:JSON.stringify(message), qos:1},function(sentMsg){
        ...
        subscribeMqttGroup=function(group_id) {
            if (group_id) {
                ...
                mqtt.subscribe({topic:'/msg/g/'+group_id,qos:1});
                mqtt.subscribe({topic:'/msg/l/'+group_id,qos:1}); // label 消息
            }
        };
        unsubscribeMqttGroup=function(group_id) {
            if (mqtt) {
                if(group_id) {
                    mqtt.unsubscribe({topic:"/msg/g/" + group_id});
                    mqtt.unsubscribe({topic:"/msg/l/" + group_id});
                }
            }
        };
        subscribeMqttUser=function(user_id){
            if (mqtt && user_id) {
                ...
                mqtt.subscribe({topic:'/msg/u/'+user_id,qos:1});
            }
        };
        unsubscribeMqttUser=function(user_id){
            if (mqtt && user_id) {
                mqtt.unsubscribe({topic:"/msg/u/" + user_id});
            }
        };
