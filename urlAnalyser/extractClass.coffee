...
processRootNode = (bodyParified, tag)->
...
########
DoubanExtract = ()->
  this.process = (page, bodyParified)->   
    console.log("DoubanExtract ");
    rootNode = processRootNode(bodyParified, '.note-content')
    
    if rootNode 
      return extractRootNode(rootNode)  
    else
      return null

WechatExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("WechatExtract ");  
    rootNode =  processRootNode(bodyParified, '.rich_media_content')
...
WeiboExtract = ()->
  this.process = (page, bodyParified)->   
    console.log("WeiboExtract ");
    rootNode =  processRootNode(bodyParified, '.WBA_content')
...
DoubanFMExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("DoubanFMExtract ");
    rootNode =  processRootNode(bodyParified, '#cont-wrapper')
...
QFMExtract = ()->
  this.process = (page, bodyParified)->   
    console.log("QFMExtract ");   
    rootNode =  processRootNode(bodyParified, '#j-body')
...
YueduFMExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("YueduFMExtract ");  
    rootNode =  processRootNode(bodyParified, '.article')
...
QQMusicExtract = ()->
  this.process = (page, bodyParified)->   
    console.log("QQMusicExtract ");   
    rootNode =  processRootNode(bodyParified, '.main_box')
...
XueqiuExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("XueqiuExtract ");  
    rootNode =  processRootNode(bodyParified, '#page-content')
...
TripadvisorExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("TripadvisorExtract ");
    rootNode =  processRootNode(bodyParified, '#BODYCON')
...
IfengExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("IfengExtract ");  
    rootNode =  processRootNode(bodyParified, '.yaow > p')
...
LinkedinExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("LinkedinExtract ");  
    rootNode =  processRootNode(bodyParified, '.pulse-article')
...
PageArticleExtract = ()->   
  this.process = (page, bodyParified)->   
    console.log("PageArticleExtract "); 
    rootNode =  processRootNode(bodyParified, '#pageletArticleContent')
...
CommonExtract = ()->   
  this.process = (page, parified)->   
    console.log("CommonExtract ");
    return commnonExtract(parified, page)
...
ExtractClass = (obj)->  
  ...
  this.extract = (arg)->   
    this.who[this.doWhat].call(this.who, arg);   

class Extract 
  process: (page) ->
    douban = { process : new DoubanExtract() };   
    wechat = { process : new WechatExtract() }; 
    weibo = { process : new WeiboExtract() }; 
    doubanfm = { process : new DoubanFMExtract() }; 
    qm = { process : new QFMExtract() }; 
    yuedufm = { process : new YueduFMExtract() }; 
    qqmusic = { process : new QQMusicExtract() }; 
    xueqiu = { process : new XueqiuExtract() }; 
    tripadvisor = { process : new TripadvisorExtract() }; 
    ifeng = { process : new IfengExtract() }; 
    linkedin = { process : new LinkedinExtract() }; 
    pagearticle = { process : new PageArticleExtract() }; 
    common = { process : new CommonExtract() };
    ...
    if page.host in specialClassNameExcludeMobileSites
      return (new ExtractClass(common)).extract(page, parified)    
    
    if ret = (new ExtractClass(douban)).extract(page, bodyParified) isnt null return ret;
    if ret = (new ExtractClass(wechat)).extract(page, bodyParified) isnt null return ret;
    if ret = (new ExtractClass(weibo)).extract(page, bodyParified) isnt null return ret;
    if ret = (new ExtractClass(doubanfm)).extract(page, bodyParified) isnt null return ret;
    if ret = (new ExtractClass(qm)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(yuedufm)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(qqmusic)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(xueqiu)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(tripadvisor)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(ifeng)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(linkedin)).extract(page, bodyParified) isnt null return ret
    if ret = (new ExtractClass(pagearticle)).extract(page, bodyParified) isnt null return ret
      
    return (new ExtractClass(common)).extract(page, parified)      

extract = (page) ->
  return Extract.process(page)
