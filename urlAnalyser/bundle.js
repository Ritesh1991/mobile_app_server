...
storeStyleInItem = function(node, type, value) {
  return $(node).attr('hotshare-' + type, value);
};
...
Score = (function() {
  ...
  Score.prototype.scale = function(s) {
    return this.value *= s;
  };

  Score.initialScoreFor = function(tagName) {
    switch (tagName) {
      case 'DIV':
        return 5;
      case 'IFRAME':
...
REGEXPS = {
  ...
  nextLink: /(next|weiter|continue|>([^\|]|$)|ﾂｻ([^\|]|$))/i,
...
specialMobileSiteForImages = ['medium.com', 'techcrunch.com'];

beforeExtractConfig = [];

this.onBeforeExtract = function(url, data) {
  return _.map(beforeExtractConfig, function(config) {
    if (config.enable(url, data)) {
      return config.extract(url, data);
    }
  });
};

textContentFor = function(node, normalizeWs) {
  var text;
  if (normalizeWs == null) {
    normalizeWs = true;
  }
  ...
  if (normalizeWs) {
    text = text.replace(REGEXPS.normalize, " ");
  }
  return text;
};

countChars = function(node, ch) {
  return textContentFor(node).split(ch).length - 1;
};

linkDensityFor = function(node) {
  var linkLength, textLength;
  linkLength = _.reduce($(node).find("a"), (function(len, n) {
    return len + n.innerHTML.length;
  }), 0);
  if (node && node.innerHTML && node.innerHTML.length) {
...
classWeight = function(node) {
  var weight;
  weight = 0;
  if (node.className) {
    if (-1 < node.className.search(REGEXPS.negative)) {
      weight -= 25;
    }
    if (-1 < node.className.search(REGEXPS.positive)) {
      weight += 25;
    }
  }
  if (node.id) {
    if (-1 < node.id.search(REGEXPS.negative)) {
      weight -= 25;
    }
    if (-1 < node.id.search(REGEXPS.positive)) {
      weight += 25;
    }
  }
  return weight;
...
fishy = function(node) {
  var contentLength, contentScore, embed, img, input, li, linkDensity, nj, p, theNode, weight;
  if (node.tagName === 'iframe' || $(node).find('iframe').length > 0) {
    console.log('fishy on iframe');
    return false;
  }
  theNode = $(node).parent() ? $(node).parent() : $(node);
  if ((node.id === 'mediaPlayer' && node.getAttribute('data-video') !== null) || theNode.find('#mediaPlayer').length > 0) {
    console.log('fishy on mediaPlayer');
    return false;
  }
  weight = classWeight(node);
  contentScore = node.score ? node.score.value : 0;
  if (10 <= countChars(node, ',')) {
    return false;
  }
  nj = $(node);
  p = nj.find("p").length;
  ...
  if (li > p && node.tagName !== "ul" && node.tagName !== "ol") {
    return true;
  }
  ...
  if ((embed === 1 && contentLength < 75) || embed > 1) {
    return true;
  }
  return false;
};

deepInsideThislayer = function(node, tagName) {
  var childrenNumber, firstChild;
  childrenNumber = $(node).children().length;
  if (childrenNumber > 0 && childrenNumber <= 2) {
    firstChild = $(node).find(':first-child').get(0);
    if (firstChild && firstChild.tagName === tagName) {
...
ensureScore = function(array, node) {
...
propagateScore = function(node, scoredList, score) {
  ...
  if (parent) {
    ...
    if (grandParent && grandParent.score) {
      ensureScore(scoredList, grandParent);
      return grandParent.score.add(score / 2);
    }
  }
};
...
scoreNode = function(node) {
  var text, unlikely;
  if (node.tagName === "IMG") {
    if ($(node).parent() && $(node).parent().hasClass('article-cover')) {
      if ($(node).parent().parent() && $(node).parent().parent().hasClass('article-header')) {
        return 99999;
      }
    }
  }
  unlikely = node.className + node.id;
  if (unlikely.search(REGEXPS.unlikelyCandidates) !== -1 && unlikely.search(REGEXPS.okMaybeItsACandidate) === -1 && node.tagName !== "BODY") {
    return 0;
  }
  ...
  return 1 + text.split(',').length + Math.min(Math.floor(text.length / 100), 3);
};

reduceScorable = function(scoredList, node) {
  var score;
...
isAcceptableSibling = function(top, sib) {
  var text, textLen, threshold;
  if (top === sib) {
    return true;
  }
  ...
  if (threshold <= scoreSibling(top, sib)) {
    return true;
  }
  if ("P" !== sib.tagName) {
    return false;
...
scoreSibling = function(top, sib) {
  if (!sib.score) {
    return 0;
  }
  if (sib.className === sib.className && sib.className !== "") {
    return sib.score.value + (top.score.value * 0.2);
  } else {
    return sib.score.value;
  }
};

removeFragments = function(node) {
  var jn;
  jn = $(node);
  jn.html(jn.html().replace(REGEXPS.killBreaks, '<br />'));
  jn.find("h1,h2,h3").find(function(n) {
    return classWeight(n) < 0;
  }).remove();
  if (removeStyle) {
    jn.find("*").removeAttr("style");
  }
  jn.find("p").filter(function() {
    return 0 === $(this).find("img").length && 0 === $(this).find("embed").length && 0 === $(this).find("object").length && 0 === $(this).find("iframe").length && 0 === textContentFor(this, false).length;
  }).remove();
  jn.find("form").filter(function() {
    return fishy(this);
  }).remove();
  jn.find("table").filter(function() {
    return fishy(this);
  }).remove();
  jn.find("ul").filter(function() {
    return fishy(this);
  }).remove();
  jn.find("div").filter(function() {
    return fishy(this);
  }).remove();
  jn.find("noscript").filter(function() {
    return fishy(this);
  }).remove();
  if (removeStyle) {
  ...
  if (jn.find("h2").length === 1) {
    jn.find("h2").remove();
  }
  return node.innerHTML = node.innerHTML.replace(/<br[^>]*>\s*<p/gi, '<p');
};

removeUnwanted = function(node) {
  var jn;
  jn = $(node);
  jn.html(jn.html().replace(REGEXPS.killBreaks, '<br />'));
  if (removeStyle) {
  ...
  if (removeStyle) {
    return jn.find("object,h1,script,link,style").remove();
  } else {
    return jn.find("object,h1,script,link").remove();
  }
};
...
scoreAndSelectTop = function(nodes) {
  var id, scored, sortArray, topId;
  scored = _.reduce(nodes, reduceScorable, []);
  sortArray = _.sortBy(scored, function(n) {
    return n.score.value;
  });
  id = scored.length - 1;
  while (id >= 0 && sortArray[id].score.value === 99999) {
    id--;
  }
  topId = scored.length - 1;
  if (id >= 0 && id !== scored.length - 1) {
    $(sortArray[id]).prepend($(sortArray[scored.length - 1]));
    topId = id;
  }
  console.log("topId=" + topId + ", scored.length=" + scored.length);
  return sortArray[topId];
};

collectSiblings = function(top) {
  return _.reduce(top.parentNode ? $(top.parentNode).children() : $(top).children(), ((function(_this) {
    return function(root, s) {
      if (isAcceptableSibling(top, s)) {
        root.appendChild(s);
      }
      return root;
    };
...
collectNodeSibling = function(node) {
  var count, el, next, text;
  el = node.nextSibling;
  count = 0;
  while (el) {
    console.log(count + ' Self.nextSibling Tag is ' + el.tagName + ' my text ' + textContentFor(node) + ' siblingNode text' + textContentFor(el) + ' siblingNode is text node ' + (el.nodeType === Node.TEXT_NODE));
    next = el.nextSibling;
    if (el.tagName === 'BR') {
      console.log('Has BR');
...
getSpecialTag = function(node, specialTagName) {
  ...
  try {
    ...
    while ($node.parent().length > 0) {
      ...
      if (tagName && tagName !== '' && tagName.toLowerCase() === specialTagName) {
        return true;
      }
      $node = $node.parent();
    }
...
cloneWithoutSibling = function(parentNode, node) {
  var $cloneParent, $parentNode;
...
extractScript = function(page, getMusic) {
  var i, item, len1, musicElement, musicInfo, newRoot, parified, ref;
...
extract = function(page) {
  var alignstyle, bodyParified, documentBody, flag, i, item, j, k, len1, len2, len3, level, newRoot, node, nodeList, p, parentPNode, parified, ref, root, rootNode, tag, tmpNode, tmpParentNode, top, treeWalker;
  parified = _.map($(page).find('*'), parify);
  documentBody = document.createElement('body');
  documentBody.innerHTML = page.innerHTML;
  bodyParified = _.map($(documentBody).find('*'), parify);
  keepImagesForSpecialMobileSite = false;
  console.log('  page.host = ' + page.host);
  if (page.host === specialMobileSiteForImages) {
    keepImagesForSpecialMobileSite = true;
  }
  for (i = 0, len1 = specialClassNameForPopularMobileSite.length; i < len1; i++) {
    tag = specialClassNameForPopularMobileSite[i];
    if (ref = page.host, indexOf.call(specialClassNameExcludeMobileSites, ref) >= 0) {
      break;
    }
    rootNode = null;
    if ($(bodyParified).find(tag).length > 0) {
      rootNode = $(bodyParified).find(tag)[0];
    } else {
      for (j = 0, len2 = bodyParified.length; j < len2; j++) {
        item = bodyParified[j];
        if (tag.indexOf('#') === 0) {
          if (item.id === tag.substr(1)) {
            rootNode = item;
            break;
          } else if (item.parentNode.id === tag.substr(1)) {
            rootNode = item.parentNode;
            break;
          }
        } else if (tag.indexOf('.') === 0) {
          if (item.className === tag.substr(1)) {
            rootNode = item;
            break;
          } else if (item.parentNode.className === tag.substr(1)) {
            rootNode = item.parentNode;
            break;
          }
        } else {
          if (item.tagName === tag.toUpperCase()) {
            rootNode = item;
            break;
          } else if (item.tagName === tag.toUpperCase()) {
            rootNode = item.parentNode;
            break;
          }
        }
    ...
    if (rootNode !== null) {
            ...
            if (!node.hasChildNodes()) {
              if (node.nodeType === Node.TEXT_NODE) {
                if ($(node).parent().length > 0) {
                  ...
                  if (alignstyle && alignstyle !== '') {
                    storeStyleInItem(node.parentNode, 'textAlign', alignstyle);
                  }
                }
                if (collectNodeSibling(node) === false) {
                  return NodeFilter.FILTER_ACCEPT;
                }
              }
            }
      ...
      parentPNode = null;
      for (k = 0, len3 = nodeList.length; k < len3; k++) {
        node = nodeList[k];
        if (!node.hasChildNodes()) {
          if (node.nodeType === Node.TEXT_NODE) {
            flag = 0;
            ...
            if (flag) {
              ...
              if (!node.parentNode || node.parentNode === parentPNode) {
              ...
              if (node.parentNode) {
                ...
                if (getSpecialTag(node, "strong") || getSpecialTag(node, "h1") || getSpecialTag(node, "h2") || getSpecialTag(node, "h3")) {
...
},{"./extract.coffee":1,"./jquery-2.1.4.min.js":3,"./url_analyser.coffee":5}],3:[function(require,module,exports){
...
var _html2data2, extract, getImagesListFromUrl, getMusicFromPage, getPossibleVideo, getStyleInItem, grabImagesInHTMLString, hostnameMapping, importColor, musicExtactorMapping, musicExtactorMappingV2, showDebug, titleRules, url_analyser, videoExtactorMapping;
...
titleRules = [
  {
    prefix: 'view.inews.qq.com',
    titleClass: 'title'
  }, {
    prefix: 'buluo.qq.com',
    titleClass: 'post-title'
  }, {
    prefix: 'wap.koudaitong.com',
    titleClass: 'custom-title.wx_template .title'
  }
];
...
hostnameMapping = [
  {
    ...
    hostname: 'card.weibo.com',
    displayName: '微博'
  }, {
    hostname: 'mil.sohu.com',
    displayName: '搜狐军事'
  }, {
...
musicExtactorMappingV2 = [
  {
    ...
    parentSelector: '',
    getMusicUrl: function(node, body) {
      return $(node).attr('audiourl');
    },
    getMusicThumbImageURL: function(node, body) {
      var nodes;
    ...
    getMusicThumbImageURL: function(node) {
      console.log('getMusicThumbImageURL None');
      return '';
    },
    getMusicSongName: function(node) {
    ...
    getMusicSingerName: function(node) {
    ...
    getMusicInfo: function(node, body) {
      var item, j, len, ref, text;
      ref = $(body).find('script');
      for (j = 0, len = ref.length; j < len; j++) {
        item = ref[j];
        text = item.innerText;
        if (text.indexOf('window.__bootstrap_data') !== -1) {
          eval(text);
    ...
    parentSelector: '',
    getMusicInfo: function(node, body) {
      var item, j, len, obj, ref, text;
      console.log(node);
      ref = $(body).find('#j-body script');
      for (j = 0, len = ref.length; j < len; j++) {
        item = ref[j];
        text = item.innerText;
        if (text.indexOf('showData:{"') !== -1) {
    ...
    getMusicInfo: function(node, body) {
      var item, j, len, player, ref, text;
      ref = $(body).find('script');
      for (j = 0, len = ref.length; j < len; j++) {
        item = ref[j];
        text = item.innerText;
    ...
    nodeSelector: '.song_infosong_info',
    parentSelector: '',
    getMusicInfo: function(node, body) {
      return {
...
this.getMusicFromNode = function(node, body) {
  ...
  for (j = 0, len = musicExtactorMappingV2.length; j < len; j++) {
    ...
    isExist = false;
    findNone = node;
    if (s.nodeSelector !== '' && node.nodeType !== Node.TEXT_NODE) {
      if (s.nodeSelector.indexOf('#') === 0) {
        if (node.id === s.nodeSelector.substr(1)) {
          isExist = true;
        }
      } else if (s.nodeSelector.indexOf('.') === 0) {
        if (node.className === s.nodeSelector.substr(1)) {
          isExist = true;
        }
      } else {
        if (node.tagName === s.nodeSelector.toUpperCase()) {
          isExist = true;
        }
      }
    } else if (s.parentSelector !== '' && $(node.parentNode).find('musicExtracted').length <= 0) {
      if (s.parentSelector.indexOf('#') === 0) {
        if (node.parentNode.id === s.parentSelector.substr(1)) {
          findNone = node.parentNode;
          isExist = true;
        }
      } else if (s.parentSelector.indexOf('.') === 0) {
        if (node.parentNode.className === s.parentSelector.substr(1)) {
          findNone = node.parentNode;
          isExist = true;
        }
      } else {
        if (node.parentNode.tagName === s.parentSelector.toUpperCase()) {
          findNone = node.parentNode;
          isExist = true;
        }
      }
    ...
    if (isExist === true) {
      ...
      if (s.getMusicInfo) {
        ...
        if (typeof s.getMusicUrl === 'function') {
          musicInfo.playUrl = s.getMusicUrl(findNone, body);
        }
        if (typeof s.getMusicThumbImageURL === 'function') {
          musicInfo.image = s.getMusicThumbImageURL(findNone, body);
        }
        if (typeof s.getMusicSongName === 'function') {
          musicInfo.songName = s.getMusicSongName(findNone, body);
        }
        if (typeof s.getMusicSingerName === 'function') {
...
this.getMusicFromScript = function(url, page) {
  var j, len, mapping;
  for (j = 0, len = musicExtactorScriptMapping.length; j < len; j++) {
    mapping = musicExtactorScriptMapping[j];
    if (url.toLowerCase().indexOf(mapping.url) === 0) {
...
getMusicFromPage = function(page) {
  var image, j, len, playUrl, s, singerName, songName;
  for (j = 0, len = musicExtactorMapping.length; j < len; j++) {
    s = musicExtactorMapping[j];
    if ($(page).find(s.musicClass).length > 0) {
      playUrl = $(page).find(s.musicUrlSelector).attr(s.musicUrlAttr);
      if (s.prefixToMusicUrl && s.prefixToMusicUrl !== '') {
        playUrl = s.prefixToMusicUrl + playUrl;
      }
      if (s.musicImgSelector && s.musicImgSelector !== '') {
...
getPossibleVideo = function(elem) {
  var imageUrl, j, len, node, playUrl, s;
  for (j = 0, len = videoExtactorMapping.length; j < len; j++) {
    s = videoExtactorMapping[j];
    if ('#' + elem.id === s.videoUrlSelector) {
      node = elem.parentNode ? elem.parentNode : elem;
    }
    if ($(node).find(s.videoClass).length > 0) {
      playUrl = $(node).find(s.videoUrlSelector).attr(s.videoUrlAttr);
      if (s.videoImgSelector && s.videoImgSelector !== '') {
...
this.seekSuitableImageFromArray = function(imageArray, callback, minimal, onlyOne) {
  var minimalWidthAndHeight;
  this.imageCounter = 0;
  this.foundImages = 0;
  if (minimal) {
  ...
  if (!this.imageResolver) {
    this.imageResolver = new Image();
  }
  imageResolver.onload = function() {
    var height, width;
    ...
    if (++imageCounter < imageArray.length) {
      return imageResolver.src = imageArray[imageCounter];
    } else {
      return callback(null, 0, 0, foundImages, imageCounter, imageArray.length);
    }
  };
  imageResolver.onerror = function() {
...
this.seekSuitableImageFromArrayAndDownloadToLocal = function(imageArray, callback, minimal, onlyOne) {
...
this.analyseUrl = function(url, callback) {
  this.iabRef = window.open(url, '_blank', 'hidden=yes');
  iabRef.addEventListener('loadstop', function() {
    showDebug && console.log('load stop');
    return getImagesListFromUrl(iabRef, url, callback);
  });
  return iabRef.addEventListener('loaderror', function() {
...
this.reAnalyseUrl = function(url, callback) {
  if (!iabRef) {
    callback(null, 0, 0);
    return;
  }
...
this.clearLastUrlAnalyser = function() {
  var iabRef;
  if (iabRef) {
    iabRef.close();
    return iabRef = void 0;
  }
};

this.seekOneUsableMainImage = function(data, callback, minimal) {
  var bgImg, imageArray, imageUrl, img, j, k, len, len1, ref, ref1;
  imageArray = [];
  if (data.imageArray) {
    ref = data.imageArray;
    for (j = 0, len = ref.length; j < len; j++) {
...
this.processInAppInjectionData = function(data, callback, minimal) {
  var bgImg, imageArray, imageUrl, img, j, k, len, len1, ref, ref1;
  imageArray = [];
  if (data.imageArray) {
    ref = data.imageArray;
  ...
  showDebug && console.log('Got images to be anylised ' + JSON.stringify(imageArray));
  if (imageArray.length > 0) {
    return seekSuitableImageFromArray(imageArray, function(url, w, h, found, index, length) {
      if (url) {
        return callback(url, w, h, found, index, length);
      } else {
        return callback(null, 0, 0, found, index, length);
      }
    }, minimal, false);
  } else {
    return callback(null, 0, 0, 0, 0, 0);
  }
};

grabImagesInHTMLString = function(data) {
  var dataSrc, documentBody, j, k, l, len, len1, len2, pattern, result, results, subString;
  documentBody = $.parseHTML(data.body);
  documentBody.innerHTML = data.body;
  $(documentBody).find('img').each(function() {
    var dataLISrc, dataSrc, src;
    dataSrc = $(this).attr('data-src');
    dataLISrc = $(this).attr('data-li-src');
    if (dataSrc && dataSrc !== '') {
      src = dataSrc;
    } else if (dataLISrc && dataLISrc !== '') {
      src = dataLISrc;
    } else {
      src = $(this).attr('src');
    }
    if (src && src !== '') {
      ...
      if ((data.imageArray.indexOf(src)) < 0) {
        return data.imageArray.push(src);
      }
    }
  });
  $(documentBody).find('input').each(function() {
    var src;
    src = $(this).attr('src');
  ...
  $(documentBody).find('div').each(function() {
    var bg_url;
      ...
      if (bg_url) {
        ...
        if (bg_url && bg_url !== '') {
          ...
          if ((data.bgArray.indexOf(bg_url)) < 0) {
            return data.bgArray.push(bg_url);
          }
        }
      }
    }
  ...
  if (result && result.length > 0) {
    ...
    for (j = 0, len = result.length; j < len; j++) {
      ...
      if ((data.imageArray.indexOf(dataSrc)) < 0 && (data.bgArray.indexOf(dataSrc)) < 0) {
  ...
  if (result && result.length > 0) {
    ...
    for (l = 0, len2 = result.length; l < len2; l++) {
      ...
      if ((data.imageArray.indexOf(dataSrc)) < 0 && (data.bgArray.indexOf(dataSrc)) < 0) {
        data.imageArray.push(dataSrc);
        results.push(showDebug && console.log('push dataSrc: ' + dataSrc));
      } else {
        results.push(void 0);
      }
    }
...
getImagesListFromUrl = function(data, url, callback) {
  var documentBody, extracted;
  if (data[0]) {
...
getStyleInItem = function(node, type) {
  return $(node).attr('hotshare-' + type);
};

_html2data2 = function(url, data, callback) {
  var appendParagraph, documentBody, extracted, imageUrl, initParagraphArray, item, j, k, l, len, len1, len2, len3, m, pageInnerText, paragraphArray, paragraphArrayTmp, previousIsImage, previousIsSpan, previousParagraph, realTitle, ref, ref1, resortedArticle, sortedImages, sortedVideos, titleRule, toBeInsertedStyleAlign, toBeInsertedText, toBeProcessed;
  pageInnerText = '';
  previousParagraph = '';
  paragraphArray = [];
  paragraphArrayTmp = [];
  initParagraphArray = function(extracted) {
    var divElement, i, j, k, ref, ref1, results;
    ...
    if (paragraphArrayTmp.length > 0) {
      for (i = j = 0, ref = paragraphArrayTmp.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
        if (!(paragraphArrayTmp[i].length === 0 || paragraphArrayTmp[i] === ' ')) {
    ...
    if (paragraphArray.length > 0) {
      ...
      results = [];
      for (i = k = 0, ref1 = paragraphArray.length - 1; 0 <= ref1 ? k <= ref1 : k >= ref1; i = 0 <= ref1 ? ++k : --k) {
        results.push(console.log('paragraphArray[' + i + ']=' + paragraphArray[i]));
      }
      return results;
    }
  };
  appendParagraph = function(resortedArticle, text, styleAlign) {
    var appendTextWithStyleAlign, isShortParagraph, lastArtical, textArray;
    isShortParagraph = false;
    appendTextWithStyleAlign = function() {
      if (!isShortParagraph) {
        if (text.trim() === '' || text.trim() === '\n') {
          return;
        }
      }
      if (styleAlign === void 0) {
        return resortedArticle.push({
          type: 'text',
          text: text
        });
      } else {
        if (styleAlign.textAlign && styleAlign.fontWeight) {
          return resortedArticle.push({
            type: 'text',
            text: text,
            layout: {
              align: styleAlign.textAlign,
              weight: styleAlign.fontWeight
            }
          });
    ...
    if (resortedArticle.length > 0) {
      ...
      if (lastArtical.type === 'text') {
        textArray = lastArtical.text.split('\n');
        if (textArray[textArray.length - 1].length < 20 && styleAlign === (lastArtical.layout ? lastArtical.layout : void 0)) {
          lastArtical.text += '\n' + text;
          if (textArray[textArray.length - 1].trim() !== '' && textArray[textArray.length - 1].trim() !== '\n') {
            return isShortParagraph = true;
          }
        } else {
          return appendTextWithStyleAlign();
        }
      } else {
        return appendTextWithStyleAlign();
      }
  ...
  for (j = 0, len = titleRules.length; j < len; j++) {
    titleRule = titleRules[j];
  ...
  for (k = 0, len1 = hostnameMapping.length; k < len1; k++) {
    item = hostnameMapping[k];
    if (data.host === item.hostname) {
  ...
  $(toBeProcessed).children().each(function(index, node) {
    ...
    styleAlign = {
      textAlign: getStyleInItem(node, 'textAlign'),
      fontWeight: getStyleInItem(node, 'fontWeight')
    };
    ...
    } else if (node.tagName === 'MUSICEXTRACTED') {
      if (toBeInsertedText && toBeInsertedText !== '') {
        appendParagraph(resortedArticle, toBeInsertedText, void 0);
        toBeInsertedText = '';
      }
    ...
    if (node.tagName === 'P') {
      ...
      if (node.tagName === 'OL') {
        ...
        if (textArray.length > 0) {
          ...
          for (i = n = 0, ref2 = textArray.length - 1; 0 <= ref2 ? n <= ref2 : n >= ref2; i = 0 <= ref2 ? ++n : --n) {
      ...
      if (node.tagName === 'SPAN' || node.tagName === 'STRONG') {
        ...
        return previousIsSpan = true;
      } else if (previousIsSpan === true) {
        toBeInsertedText += text;
        previousIsSpan = false;
        return text = '';
      } else if (toBeInsertedText.length < 20 && styleAlign === toBeInsertedStyleAlign) {
        if (toBeInsertedText.length > 0) {
          toBeInsertedText += '\n';
        }
        return toBeInsertedText += text;
      } else {
        if (toBeInsertedText.length > 0) {
          appendParagraph(resortedArticle, toBeInsertedText, toBeInsertedStyleAlign);
        }
  ...
  if (sortedImages < 1 && sortedVideos < 1) {
...
url_analyser.getImagesListFromUrl = getImagesListFromUrl;

url_analyser.analyserHTML = _html2data2;

module.exports = url_analyser;
